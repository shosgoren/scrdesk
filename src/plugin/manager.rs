...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "scrdesk".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/scrdesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "SCRDESK wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "SCRDESK wants to uninstall the plugin",
    )
}
