...
pub mod server {
    ...
    async fn run_ipc_client() {
        ...
        match ipc::connect(1000, postfix).await {
            Ok(mut stream) => {
                let mut timer =
                    crate::scrdesk_interval(tokio::time::interval(Duration::from_secs(1)));
                let mut nack = 0;
                loop {
                    tokio::select! {
                        res = stream.next() => {
                            ...
                            match result {
                                Ok(stream) => {
                                    ...
                                    tokio::spawn(async move {
                                        ...
                                        let mut timer = crate::scrdesk_interval(tokio::time::interval(Duration::from_secs(1)));
                                        let mut nack = 0;
